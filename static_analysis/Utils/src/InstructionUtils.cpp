...
bool InstructionUtils::same_types(Type* ty0, Type* ty1, bool wild_intp) {
	...
	if (wild_intp) {
		...
		if (InstructionUtils::isPrimitiveTy(ty0,8) || InstructionUtils::isPrimitiveTy(ty1,8)) {
			return (!dyn_cast<StructType>(ty0) && !dyn_cast<StructType>(ty1));
		}
	}
...
GetElementPtrInst *InstructionUtils::createSubGEP(GEPOperator* gep,unsigned i) {
	...
	auto *ty = llvm::dyn_cast<llvm::PointerType>(
		gep->getPointerOperandType()->getScalarType())->getElementType();
	return GetElementPtrInst::Create(ty/*PointeeType*/, gep->getPointerOperand()/*Value *Ptr*/, 
...
std::vector<FieldDesc*> *InstructionUtils::getCompTyDesc(DataLayout *dl, StructType *ty) {
	...
	if (dyn_cast<ArrayType>(ty)) {
		...
		dbgs() << "Cannot recognize the StructType: " << InstructionUtils::getTypeStr(ty) << "\n";
		assert(false);
	}
#ifdef DEBUG_GET_TY_DESC
