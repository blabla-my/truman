...
const std::set<std::string> KernelFunctionChecker::dma_functions{"dma_alloc_coherent"};
...
bool KernelFunctionChecker::is_dma_function(const Function *targetFunction) {
    if(targetFunction->hasName()) {
        std::string func_name = targetFunction->getName().str();
        for (const std::string &curr_func : KernelFunctionChecker::dma_functions) {
            if (func_name.find(curr_func.c_str()) != std::string::npos) {
                return true;
            }
        }
