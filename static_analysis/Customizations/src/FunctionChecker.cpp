...
void* FunctionHandler::handleFunction(CallInst &callInst, Function *targetFunction, void *private_data,
    ...
    if (this->targetChecker->is_dma_function(targetFunction)) {
#ifdef DEBUG_HANDLE_FUNCTION
        llvm::dbgs() << "FunctionHandler::handleFunction(): Processing dma function: " << currFuncName << "\n";
#endif
        is_handled = true;
        return callback->handleDMAFunction(callInst, targetFunction, private_data);
    }
