...
Type *getLoadedPointeeTy(Instruction *targetInstr) {
    if (targetInstr && dyn_cast<LoadInst>(targetInstr)) {
        ...
        if (ptrTy->isPointerTy()) {
            ...
            if (InstructionUtils::isPrimitiveTy(ty) || !dyn_cast<StructType>(ty)) {
...
int AliasObject::getAllArrayElm(long fid, std::set<TypeField*> &res) {
    ...
    while (obj->parent) {
        ...
        obj = obj->parent;
        if (dyn_cast<ArrayType>(obj->targetType)) {
            has_seq = true;
        }
    }
    if (!dyn_cast<ArrayType>(this->targetType) && !has_seq) {
    ...
    for (AliasObject *e : eqs) {
        if (dyn_cast<ArrayType>(e->targetType)) {
            std::set<long> eqfs = e->getAllAvailableFields();
            eqfs.insert(-1);
            for (long t : eqfs) {
        ...
        for (ObjectPointsTo *obj : livePtos) {
            if (obj->fieldId == srcfieldId && obj->targetObject) {
                ...
                if (obj->dstfieldId == 0 && obj->targetObject->isHeapLocation() && 
                    expObjTy && dyn_cast<StructType>(expObjTy) && InstructionUtils::isPrimitiveTy(obj->targetObject->targetType)) 
                {
#ifdef DEBUG_CHANGE_HEAPLOCATIONTYPE
...
void AliasObject::createFieldPointee(long fid, std::set<std::pair<long, AliasObject*>> &dstObjects, InstLoc *currInst, InstLoc *siteInst) {
    ...
    if (real_ty->isFunctionTy()) {
        ...
        if (enableXentryImpObjShare) {
            newObj = getSharedObjFromCache(real_ty);
#ifdef DEBUG_FETCH_POINTS_TO_OBJECTS
...
AliasObject *AliasObject::createEmbObj(long fid, Value *v, InstLoc *loc) {
    ...
    if (!newObj || !InstructionUtils::same_types(newObj->targetType,ety)) {
        ...
        if (v) {
            newObj = createOutsideObj(v,loc);
        }else {
            newObj = createOutsideObj(ety);
        }
        if (newObj) {
#ifdef DEBUG_CREATE_EMB_OBJ
...
AliasObject *AliasObject::createHostObj(Type *hostTy, long field, InstLoc *loc) {
    if (!hostTy || !dyn_cast<StructType>(hostTy)) {
#ifdef DEBUG_CREATE_HOST_OBJ
        dbgs() << "AliasObject::createHostObj(): !hostTy || !dyn_cast<StructType>(hostTy)\n";
#endif
        return nullptr;
    }
#ifdef DEBUG_CREATE_HOST_OBJ
    ...
    AliasObject *hobj = createOutsideObj(hostTy);
    if (hobj) {
        //NOTE: the "auto_generated" property should be same between host and emb objects, but "createOutsideObj" by default
        //set it to true.
...
CandStructInf *inferContainerTy(Module *m, Value *v, Type *ty, long bitoff) {
    ...
    for (User *u : v->users()) {
        ...
        for (Type *t : candTy1) {
            ty1 = t;
            if (dyn_cast<StructType>(ty1) || (ty1->isPointerTy() && !ty1->getPointerElementType()->isIntegerTy())) {
                break;
            }
        }
        if (!ty1) {
        ...
        std::vector<CandStructInf*> *c = getStructFrom2Fields(&dl,ty,n0,ty1,n1,resOff,m);
...
bool isSharedTy(Type *ty) {
    ...
    return (sharedObjTyStrs.find(n) != sharedObjTyStrs.end());
}

int addToSharedObjCache(OutsideObject *obj) {
    if (!obj || !currEntryFunc ||!obj->targetType) {
#ifdef DEBUG_SHARED_OBJ_CACHE
        dbgs() << "addToSharedObjCache(): for the obj: (!obj || !currEntryFunc ||!obj->targetType)\n";
#endif
        return 0;
    }
    ...
    << currEntryFunc->getName().str() << "\n";
#endif
    sharedObjCache[obj->targetType][currEntryFunc].insert(obj);
    return 1;
}
...
OutsideObject *getSharedObjFromCache(Type *ty) {
    if (!ty || !currEntryFunc || !isSharedTy(ty)) {
        return nullptr;
    }
    for (auto &c : sharedObjCache) {
        if (!InstructionUtils::same_types(ty,c.first)) {
            continue;
        }
        for (auto &e : c.second) {
            if (e.first != currEntryFunc) {
...
OutsideObject *getSharedObjFromCache(Value *v) {
    ...
    << " currEntryFunc: " << currEntryFunc->getName().str() << "\n";
#endif
    if (!ty || !currEntryFunc) {
#ifdef DEBUG_SHARED_OBJ_CACHE
        dbgs() << "getSharedObjFromCache(): (!ty || !currEntryFunc)\n";
#endif
        return nullptr;
    }
    if (!isSharedTy(ty)) {
