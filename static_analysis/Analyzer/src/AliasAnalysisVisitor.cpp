...
// #define DEBUG_HANDLE_INLINE_POINTER
    ...
    OutsideObject* AliasAnalysisVisitor::createOutsideObj(Value *p, Instruction *I, bool taint) {
        ...
        if (DRCHECKER::enableXentryImpObjShare) {
            ...
            if (p->getType() && p->getType()->isPointerTy() && dyn_cast<StructType>(p->getType()->getPointerElementType())) {
    ...
    int AliasAnalysisVisitor::matchPtoTy(Type *srcTy, PointerPointsTo *pto, Instruction *I, bool create_host) {
        ...
        if (dyn_cast<ArrayType>(srcTy) && dyn_cast<StructType>(pTy)) {
            Type *bty = nullptr, *bty0 = dyn_cast<ArrayType>(srcTy)->getElementType();
            int sz = -1, sz0 = dyn_cast<ArrayType>(srcTy)->getNumElements();
        ...
        if (dstfieldId || !dyn_cast<StructType>(srcTy)) {
            return 0;
        }
...
#endif
            InstLoc *loc = (I ? new InstLoc(I,this->currFuncCallSites) : nullptr);
            if (!dyn_cast<StructType>(obj->targetType)) {
#ifdef DEBUG_UPDATE_POINTSTO
    ...
    void AliasAnalysisVisitor::updatePointsToObjects(Value *p, AliasObject *obj, InstLoc *propInst, long dfid, bool is_weak) {
        ...
        // std::set<PointerPointsTo*> ori_ptos;
        // ori_ptos.insert(pto);
        ptos.insert(pto);
        // for (auto &entry_arg: this->currState.entry_args_set) {
        //     // auto *src_type = p->getType();
        //     auto *src_type = InstructionUtils::inferPointeeTy(p);
        //     auto *arg = std::get<2>(entry_arg);
        //     auto *arg_type = std::get<0>(entry_arg);
        //     llvm::dbgs() << "src_type: " << *src_type << " arg_type:" << *arg_type << "\n";
        //     if (arg_type != src_type) {
        //         continue;
        //     }
        //     llvm::dbgs() << "value: " << *p << " arg:" << *arg << "\n";
        //     this->updatePointsToObjects(arg, &ori_ptos, false);

        //     ptos.insert(std::get<4>(entry_arg));
        // }
    ...
    void AliasAnalysisVisitor::updatePointsToObjects(Value *srcPointer, std::set<PointerPointsTo*> *newPointsToInfo, bool free = true) {
        ...
        // if (flag) {
        //     for (auto &entry_arg: this->currState.entry_args_set) {
        //         // auto *src_type = InstructionUtils::inferPointeeTy(srcPointer);
        //         auto *src_type = srcPointer->getType();
        //         auto *arg = std::get<2>(entry_arg);
        //         auto *arg_type = std::get<0>(entry_arg);
        ...
        //             continue;
        //         }
        //         auto *obj = pto->targetObject;
        //         this->updatePointsToObjects(arg, &ori_info, false);
...
// #endif
...
#ifdef DEBUG_UPDATE_POINTSTO
        for (auto *curr_points_to: *existingPointsTo) {
            llvm::dbgs() << "++ PTO: ";
            curr_points_to->print(llvm::dbgs());
        }
#endif
    ...
    std::set<PointerPointsTo*> *AliasAnalysisVisitor::makePointsToCopy_emb(Instruction *propInstruction, Value *srcPointer, Value *resPointer,
        ...
        for (PointerPointsTo *currPointsToObj : *srcPointsTo) {
            ...
            if (!host_type || !dyn_cast<StructType>(host_type)) {
#ifdef DEBUG_GET_ELEMENT_PTR
...
#endif
            if (!ety || !dyn_cast<StructType>(ety)) {
#ifdef DEBUG_GET_ELEMENT_PTR
...
void AliasAnalysisVisitor::visitCastInst(CastInst &I) {
    ...
    int AliasAnalysisVisitor::bit2Field(Instruction *propInst, GEPOperator *I, PointerPointsTo *pto, unsigned bitWidth, long index) {
        ...
        while (true) {
...
#endif
            if (!targetObjTy || !dyn_cast<StructType>(targetObjTy)) {
#ifdef DEBUG_GET_ELEMENT_PTR
            ...
            tydesc = InstructionUtils::getCompTyDesc(dl,dyn_cast<StructType>(targetObjTy));
            if (!tydesc) {
#ifdef DEBUG_GET_ELEMENT_PTR
            ...
            if (dstfield == -1) {
                ...
                assert(dyn_cast<ArrayType>(targetObjTy));
                Type *ety = dyn_cast<ArrayType>(targetObjTy)->getElementType();
    ...
    //Given a pto and a *StructType*, return the matched sub- or super- obj of the pointee obj.
    AliasObject *AliasAnalysisVisitor::getObj4Copy(PointerPointsTo *pto, StructType *ty, Instruction &I) {
        if (!pto || !ty) {
            return nullptr;
        }
    ...
    PointerPointsTo *AliasAnalysisVisitor::copyObj(Value *dstPointer, PointerPointsTo *srcPto, Type *ty, Instruction &propInst) {
        ...
        StructType *cty = dyn_cast<StructType>(ty);
    ...
    void AliasAnalysisVisitor::handleDMAFunction(llvm::CallInst &I) {
        return;
#ifdef DEBUG_CALL_INSTR
        llvm::dbgs() << "AliasAnalysisVisitor::handleDMAFunction(): Processing DMA function.\n";
#endif
        auto *phy_dma_addr = 
            llvm::dyn_cast<llvm::Instruction>(InstructionUtils::stripAllCasts(I.getArgOperand(2), true));
        // auto *phy_dma_addr = &I;
        auto *size_arg = I.getArgOperand(1);
        auto *src_points_to = this->getPtos(&I, phy_dma_addr);
        auto *loc = new InstLoc(&I, this->currFuncCallSites);
        auto *ty = InstructionUtils::inferPointeeTy(phy_dma_addr);
        std::set<PointerPointsTo*> new_ptos;
        if (src_points_to && !src_points_to->empty()) {
            for (auto *pto: *src_points_to) {
                pto->print(llvm::dbgs());
            }
        }
        if (!new_ptos.empty()) {
            this->updatePointsToObjects(&I, &new_ptos, false);
        } else {
            auto *obj = new HeapLocation(*phy_dma_addr, ty, this->currFuncCallSites, size_arg, false);
            obj->setAsTaintSrc(loc);
            this->updatePointsToObjects(phy_dma_addr, obj, loc);
            auto *target_points_to_map = this->currState.getPointsToInfo(this->currFuncCallSites);
            auto *points_to_set = (*target_points_to_map)[phy_dma_addr];
            for (auto *points_to: *points_to_set) {
                auto *value = points_to->targetObject->getValue();
                if (!value && !points_to->targetObject->targetType) {
                    continue;
                }
                // TaintTag *atag = nullptr;
                // if (value) {
                //     atag = new TaintTag(-1, value, true, (void*)points_to->targetObject);
                // } else {
                //     atag = new TaintTag(-1, points_to->targetObject->targetType, true, (void*)points_to->targetObject);
                // }
                // auto *atf = new TaintFlag(nullptr, true, atag);
                // atf->is_inherent = true;
                // if (points_to->targetObject->addAllContentTaintFlag(atf)) {
                    TaintTag *tag = nullptr;
                    if (value) {
                        tag = new TaintTag(points_to->dstfieldId, value, true, (void*)points_to->targetObject);
                    } else {
                        tag = new TaintTag(points_to->dstfieldId, points_to->targetObject->targetType, true, (void*)points_to->targetObject);
                    }
                    auto *newFlag = new TaintFlag(nullptr, true, tag);
                    newFlag->is_inherent = true;
                    points_to->targetObject->addFieldTaintFlag(points_to->dstfieldId, newFlag);
                // }
                points_to->targetObject->is_taint_src = true;
                // points_to->targetObject->setAsTaintSrc(loc, true);
                // points_to->targetObject->updateFieldPointsTo(points_to->dstfieldId, points_to_set, loc, 0);
            }
        }
    }
    ...
    void AliasAnalysisVisitor::handleMemcpyFunction(std::vector<long> &memcpyArgs, CallInst &I) {
        ...
        StructType *cty = ty ? dyn_cast<StructType>(ty) : nullptr;
#ifdef DEBUG_CALL_INSTR
    ...
    VisitorCallback* AliasAnalysisVisitor::visitCallInst(CallInst &I, Function *currFunc,
        ...
        if (targetChecker->is_dma_function(currFunc)) {
#ifdef DEBUG_CALL_INSTR
            llvm::dbgs() << "AliasAnalysisvisitor::visitCallInst(): This is a dma function.\n";
#endif
            this->handleDMAFunction(I);
        }
...
#endif
                        // this->updatePointsToObjects(&I, newPointsToInfo);
                        auto *phy_dma_addr = I.getArgOperand(2);
                        this->updatePointsToObjects(
                            InstructionUtils::stripAllCasts(phy_dma_addr, true), newPointsToInfo);
                        auto *inst_loc = new InstLoc(phy_dma_addr, callSiteContext);
                    }
                } else {
#ifdef DEBUG_CALL_INSTR
