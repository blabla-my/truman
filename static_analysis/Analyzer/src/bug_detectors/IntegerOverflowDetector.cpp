...
#define DEBUG_VISIT_BIN_OP
    ...
    void IntegerOverflowDetector::visitBinaryOperator(BinaryOperator &I) {
        ...
        std::set<AliasObject *> dst_objects_0;
        std::set<AliasObject *> dst_objects_1;
        PointsToUtils::getAllAliasObjects(currState, currFuncCallSites, I.getOperand(0), dst_objects_0);
        PointsToUtils::getAllAliasObjects(currState, currFuncCallSites, I.getOperand(1), dst_objects_1);
        for (auto &dst: dst_objects_0) {
            targetValues.insert(dst->getValue());
        }
        for (auto &dst: dst_objects_1) {
            targetValues.insert(dst->getValue());
        }
        llvm::dbgs() << "callsites:\n";
        ...
        for (auto currVal : targetValues) {
            std::set<TaintFlag*> *srcTaintInfo =
                TaintUtils::getTaintInfo(this->currState, this->currFuncCallSites, currVal);
            llvm::dbgs() << "curr_val: " << *currVal << "\n";
