...
virtfuzz::Message generateClockMessage(std::mt19937 &rng) {
    virtfuzz::Message message;

	message.set_type(virtfuzz::CLOCK);
...
virtfuzz::MessageSequence generateMessages(std::mt19937 &rng) {
    ...
    } else if (dma_choice < 15) {
#ifdef STATISTICS
		updateCounters("TOTAL");
#endif
		*message_sequence.add_messages() = generateClockMessage(rng);
	}
...
virtfuzz::Message generateMessage(std::mt19937 &rng) {
    ...
    } else if (io_choice < 95) {
        message = generateMessageRandom(rng, true);
    } else {
		message = generateClockMessage(rng);
	}
